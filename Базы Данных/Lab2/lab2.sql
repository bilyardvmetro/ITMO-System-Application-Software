...
Затем дали доп таск: вывеси количество отличников по учебным годам
и добавить строку, в которой будет общее количество отличников
*/

SELECT УЧЕБНЫЙ_ГОД, COUNT(*)
FROM Н_УЧЕНИКИ
JOIN Н_ВЕДОМОСТИ ON Н_ВЕДОМОСТИ.ЧЛВК_ИД = Н_УЧЕНИКИ.ИД
JOIN Н_ПЛАНЫ ON Н_ПЛАНЫ.ИД = Н_УЧЕНИКИ.ПЛАН_ИД
WHERE Н_ВЕДОМОСТИ.ОЦЕНКА IN ('зачет', '5')
GROUP BY ROLLUP(УЧЕБНЫЙ_ГОД)
