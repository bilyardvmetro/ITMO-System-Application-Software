...
SELECT Н_УЧЕНИКИ.ГРУППА, Н_ЛЮДИ.ФАМИЛИЯ,
...
GROUP BY Н_УЧЕНИКИ.ГРУППА, Н_ЛЮДИ.ФАМИЛИЯ,
...
HAVING AVG(CAST(Н_ВЕДОМОСТИ.ОЦЕНКА AS INTEGER)) >= (
    ...
    FROM (
		...
		/* оформить с помощью WITH (common table expression)*/
    ) AS МАКС_ОЦЕНКА
);

/* Задание 6
Получить список студентов, зачисленных
до первого сентября 2012 года на первый
курс очной или заочной формы обучения (специальность: 230101).
